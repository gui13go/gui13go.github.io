...
- A terminal emulator (e.g., Terminal on macOS/Linux, PowerShell or Command Prompt on Windows).
- Access to a remote Linux server (e.g., Ubuntu or Kali Linux).
- The remote server’s IP address and your login credentials.

---

## Exercise 0: Find Out the IP Address

First, identify the IP address of the destination machine:
...
## -C adds a human-readable "comment" to the end of your public key (stored in ~/.ssh/id_ed25519.pub)
## Note: ed25519 is preferred over rsa because it is faster, more secure, and produces smaller keys.
...
# Create SSH config on client machine for shortcuts
cat << 'EOF' >> ~/.ssh/config
...
- **Connection timed out:** Check firewall status (`sudo ufw status` on both client and host).
- **Permission denied:** Verify `~/.ssh` directory permissions (`700`) and key file permissions (`600` for private, `644` for public).
- **Connection refused:** Verify the SSH daemon is running using `sudo systemctl status ssh`.
...
# Connect to the remote server via tailnet
ssh server-user@$TAILNET_SERVER_IP

# Just like before, you may configure a shortcut in ~/.ssh/config
nano ~/.ssh/config
...
- **Power State:** If the host machine goes to sleep, the Tailscale service will pause because the operating system is not running. It automatically resumes when the machine wakes up.
- **Key Expiry:** If your Tailscale account is configured with key expiry, the node key might expire, requiring re-authentication (common for ephemeral nodes or if configured in the admin console).

---

## Exercise 5: Server Hardening

Securing the SSH daemon on the server is critical. Edit `/etc/ssh/sshd_config` on your remote server to restrict access:
...
After modifying the configuration, test and restart the SSH daemon:
```bash
sudo sshd -t
...
## Exercise 7: Advanced Connectivity (Bastions & Port Forwarding)

When a server is only accessible through an intermediary (a bastion host), use **ProxyJump**. ProxyJump is generally preferred over older `-W` or `-L` methods because it is simpler to configure and more secure, ensuring the jump host cannot decrypt end-to-end traffic:
...
Sometimes you need to access a web service running on the remote server (e.g., a database GUI or monitoring tool) that is not exposed to the public internet. You can tunnel this traffic through your SSH connection via **Port Forwarding**:
...
Regularly audit your SSH configuration for cryptographic and configuration weaknesses using tools like `ssh-audit`:
